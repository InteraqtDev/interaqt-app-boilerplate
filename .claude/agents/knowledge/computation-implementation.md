...
- When accessing object-type properties (e.g., API event's `data`), check `attributes` field in {module}.data-design.json for structure
...
**⚠️ IMPORTANT: Querying Polymorphic Entity Relations**

When a property computation needs to query a relation pointing to an polymorphic entity, treat it as a normal relation:

```typescript
// Example: CreditBalance depends on polymorphic entity relation
const UserCreditProfile = Entity.create({
...
const creditBalanceProperty = UserCreditProfile.properties.find(p => p.name === 'creditBalance')!
creditBalanceProperty.computation = Custom.create({
  name: 'CreditBalanceCalculator',
  dataDeps: {
    _current: {
      type: 'property',
      attributeQuery: [
        ...
        ['user', {
          attributeQuery: [
            ...
            ['creditConsumptions', { attributeQuery: ['amount'] }]  // ✅ Query polymorphic entity relation normally
          ]
        }]
      ]
    }
  ...
  compute: async function(dataDeps, record) {
    const user = dataDeps._current?.user
    if (!user) return 0
    
    const totalRecharge = (user.creditRecharges || []).reduce((sum, r) => sum + (r.amount || 0), 0)
    const totalConsumption = (user.creditConsumptions || []).reduce((sum, c) => sum + (c.amount || 0), 0)
    
    return totalRecharge - totalConsumption  // Works for all concrete consumption types
  },
  getInitialValue: function() {
    return 0
  }
...
**Key Point**: Even if `CreditConsumptionRecord` is polymorphic with no concrete children yet, include the relation in attributeQuery. 
