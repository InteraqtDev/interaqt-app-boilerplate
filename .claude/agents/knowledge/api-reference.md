...
- `config.type` (string, required): Property type, options: 'string' | 'number' | 'boolean' | 'object'
...
const Notification = Entity.create({
  ...
  computation: Transform.create({
    ...
    callback: async function(this: Controller, mutationEvent) {
      // CRITICAL: Only use id from mutationEvent, then query for complete data
      const orderId = mutationEvent.record.id;
      
      // Fetch complete order data with all required fields
      const order = await this.system.storage.findOne('Order',
        this.globals.MatchExp.atom({ key: 'id', value: ['=', orderId] }),
        undefined,
        ['id', 'customerEmail', 'orderNumber']
      );
