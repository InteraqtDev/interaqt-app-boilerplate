    ...
    for (const taskEntity of taskEntities) {
        ...
        const statusState = StateNode.create({
            ...
            computeValue: (lastValue, mutationEvent) => {
                // Status flow:
                // - pending -> queued (workflow picks up)
                // - queued -> processing (sync: before API call; async: after submit returns taskId)
                // - queued -> failed (external API call failure)
                // - processing -> completed|failed
                // - completed|failed -> queued (retry)
...
export type ExternalTaskStatus = 'pending' | 'queued' | 'processing' | 'completed' | 'failed';
...
// - completed|failed -> queued (retry)
//
// ALL events MUST include entityId (API Call ID) since Temporal workflow context is available
// externalId is included in 'processing' event for debugging (sync: apiCallId, async: external taskId)
