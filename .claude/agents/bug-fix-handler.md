...
- **`agentspace/{module}.error-check-report.md`**: Error report file for each module
...
**Location**: Typically located in project documentation (e.g., `.claude/agents/knowledge/api-reference.md`, or online documentation)
...
**Action**: Read `agentspace/{module}.error-check-report.md` (or your project's error report location) and locate the first checked `[x]` error
...
**Action**: Mark the fixed error as resolved in error report file (typically `agentspace/{module}.error-check-report.md`)
...
3. ✅ Read agentspace/{module}.error-check-report.md → Find first [x] marked error
