...
In `agentspace/{module}.data-design.json`, integration event entities MUST have:
...
2. File to delete: `agentspace/{module}.error-check-report.md`
...
Before starting any checks, create a comprehensive checklist document in `agentspace/{module}.error-check-report.md`:
...
- [ ] ERROR_DC_008: **CRITICAL**: External event entity missing required fields (eventType, entityId, status, createdAt, data). Note: entityId is REQUIRED for all events since Temporal workflow provides API Call ID. externalId is optional (for debugging in 'processing' event).
- [ ] ERROR_DC_009: **CRITICAL**: APICall entity missing required fields (status, externalId, requestParams, responseData, createdAt, startedAt, attempts, completedAt, error)
...
- [ ] ERROR_DC_019: **CRITICAL**: APICall entity properties (status, externalId, responseData, startedAt, completedAt, error, attempts) not using `"method": "statemachine"` computation
...
- ✅ CORRECT: APICall entity has status, externalId, requestParams, responseData, createdAt, startedAt, attempts, completedAt, error properties
- ✅ CORRECT: APICall entity properties (status, externalId, responseData, startedAt, completedAt, error, attempts) use `"method": "statemachine"` computation
...
- ❌ WRONG: APICall entity missing status/externalId/requestParams/responseData/createdAt/startedAt/attempts/completedAt/error properties
- ❌ WRONG: Integration Event entity missing entityId property (required for all events)
...
- [ ] ERROR_DD_002: Data analysis not following systematic approach from `.claude/agents/knowledge/data-analysis.md`
...
- ✅ MUST have: All entities from `agentspace/{module}.data-concepts.json` with `entityType: "api-call"` are analyzed
...
- [ ] ERROR_CI_022: Error document not created in `agentspace/errors/` after repeated failures
...
- [ ] ERROR_PR_002: `agentspace/{module}.business-rules-and-permission-control-implementation-plan.json` missing
...
grep -i "webhook\|callback\|external system\|role.*System" agentspace/{module}.requirements-analysis.json

# Check for automatic system language (ERROR_RA_005)
grep -i "automatically\|system will\|auto-" agentspace/{module}.requirements-analysis.json

# Check for integration events in interaction creates arrays (ERROR_ID_004)
grep -A 10 "\"creates\":" agentspace/{module}.interactions-design.json | grep "Event\""

# Check for role="System" in interactions (ERROR_ID_003)
grep "\"role\".*\"System\"" agentspace/{module}.interactions-design.json
...
grep -A 20 "\"name\".*\".*Event\"" agentspace/{module}.data-design.json | grep -E "isIntegrationEvent|creationInteractions|api-event"
...
if [[ $(cat .currentmodule) != "basic" ]]; then
  grep -A 5 '"name".*"User"' agentspace/{module}.data-concepts.json
...
grep -B 2 -A 10 '"name".*"User.*Profile"\|"User.*Settings"\|"User.*Stats"' agentspace/{module}.data-concepts.json | grep -L 'entityType.*user-profile'

# Check for APICall entities missing entityType field (ERROR_DC_010)
grep -B 2 -A 10 '"name".*"APICall"' agentspace/{module}.data-concepts.json | grep -L 'entityType.*api-call'

# Check for APICall entities missing required properties (ERROR_DC_009)
grep -A 50 '"name".*"APICall"' agentspace/{module}.data-concepts.json | grep -E '"status"|"externalId"|"requestParams"|"responseData"|"createdAt"|"startedAt"|"attempts"|"completedAt"|"error"'

# Check for Integration Event entities missing entityId (ERROR_DC_008)
# entityId is REQUIRED for all events since Temporal workflow provides API Call ID
grep -A 30 '"name".*"Event"' agentspace/{module}.data-concepts.json | grep '"entityId"'

# Check for APICall relation cardinality (ERROR_DC_013)
# APICall relations to business entities must be 1:n
grep -B 5 -A 10 'APICallRelation' agentspace/{module}.data-concepts.json | grep '"type"'

# Check for business properties depending directly on Event (ERROR_DC_015)
# Should depend on APICall, not Event
grep -A 5 '"computed": true' agentspace/{module}.data-concepts.json | grep -B 3 'Event' | grep -v 'APICall'

# Check for APICall properties using wrong computation method (ERROR_DC_019)
# Should use "statemachine" for status, externalId, responseData, startedAt, completedAt, error
grep -B 5 -A 10 '"name".*"APICall"' agentspace/{module}.data-concepts.json | grep -A 5 '"status"\|"externalId"\|"responseData"\|"startedAt"\|"completedAt"\|"error"' | grep '"method"'

# Check for user profile entities in interaction creates (ERROR_ID_005)
grep -B 5 -A 10 '"creates"' agentspace/{module}.interactions-design.json | grep -E 'Profile|Settings|Stats'

# Check for APICall entity lifecycle.creation.type (ERROR_DD_013)
# Should be "mutation-derived", not "interaction-created"
grep -B 5 -A 15 '"name".*".*APICall"' agentspace/{module}.data-design.json | grep -A 5 '"creation"' | grep '"type"'

# Check for APICall entity relatedBusinessEntity field (ERROR_DD_014)
grep -B 5 -A 15 '"name".*".*APICall"' agentspace/{module}.data-design.json | grep '"relatedBusinessEntity"'
...
3. **Commit the report**:
   ```bash
   git add agentspace/{module}.error-check-report.md
...
grep -A 10 '"name".*"User"' agentspace/{module}.data-concepts.json
...
grep -B 2 -A 5 '"UserGiftProfile"\|"UserPreferences"\|"UserStats"' agentspace/{module}.data-concepts.json | grep 'entityType'
```
...
```json
// In agentspace/{module}.data-concepts.json - WRONG! Missing APICall entity
{
  "entities": [
    {
      "name": "Greeting",
...
```json
// In agentspace/{module}.data-concepts.json - CORRECT!
{
  "entities": [
    {
      "name": "Greeting",
    ...
    },
    {
      ...
      "properties": [
        ...
        },
        {
          ...
          "computation": {
            "method": "statemachine",
            "description": "Extracted from VolcTTSEvent.externalId when status is 'processing'. For sync APIs: apiCallId; For async APIs: external taskId. Preserved for debugging.",
        ...
        {"name": "createdAt", "type": "timestamp"},  // ✅ Not "callTime"
        {
          ...
          "computation": {
            "method": "statemachine",
            "description": "Set to current timestamp when status transitions to 'queued' (from 'pending' or 'failed')",
        ...
        },
        {
          ...
          "computation": {
            "method": "statemachine",
            "description": "Increments +1 when status transitions to 'failed' (from 'queued' or 'processing') or 'completed' (from 'processing')",
            "dependencies": ["VolcTTSEvent.status"]
          },
          "defaultValue": 0
        },
    ...
    },
    {
      ...
      "properties": [
        ...
        {"name": "entityId", "type": "string", "required": true},  // ✅ REQUIRED for all events - API Call ID for state machine matching
        {"name": "externalId", "type": "string", "required": false},  // ✅ Optional - for debugging in 'processing' event
...
**❌ WRONG: Integration event entity in agentspace/{module}.data-design.json**
```json
{
  "name": "VolcTTSEvent",
...
**✅ CORRECT: Integration event entity in agentspace/{module}.data-design.json**
```json
{
  "name": "VolcTTSEvent",
...
**❌ WRONG: APICall entity in agentspace/{module}.data-design.json**
```json
{
  "name": "VolcTTSCall",
...
**✅ CORRECT: APICall entity in agentspace/{module}.data-design.json**
```json
{
  "name": "VolcTTSCall",
