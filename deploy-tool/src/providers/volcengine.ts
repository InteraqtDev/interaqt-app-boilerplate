...
/**
 * 火山引擎 Provider
 * 使用火山引擎 VKE (Volcengine Kubernetes Engine) 进行部署
 */
export class VolcengineProvider extends BaseProvider {
  ...
  validateConfig(config: FinalConfig): ValidationResult {
    ...
    // 验证 provider 字段
    if (config.provider && config.provider !== "volcengine") {
      errors.push(`配置的 provider 为 ${config.provider}，但当前使用 VolcengineProvider`);
    }
    ...
    for (const [componentName, component] of Object.entries(config.components)) {
      ...
      if (deploymentType === "local") {
        errors.push(`组件 ${componentName} 的 deploymentType 为 local，` + `volcengine provider 只支持 'container'`);
      }
    }
  ...
  getK8sContext(): string | null {
    // 火山引擎 VKE 集群的 context 名称
    // 可以通过环境变量配置
    return process.env.VOLCENGINE_K8S_CONTEXT || null;
  }
